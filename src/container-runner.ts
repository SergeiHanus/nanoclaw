...
import { CONTAINER_RUNTIME_BIN, hostGatewayArgs, readonlyMountArgs, runtimeExtraArgs, stopContainer } from './container-runtime.js';
...
export async function buildAgentGroupImage(agentGroupId: string): Promise<void> {
  ...
  try {
    const buildSecOpts = runtimeExtraArgs().join(' ');
    execSync(`${CONTAINER_RUNTIME_BIN} build ${buildSecOpts} -t ${imageTag} -f ${tmpDockerfile} .`, {
      cwd: DATA_DIR,
      stdio: 'pipe',
      timeout: 300_000,
    });
